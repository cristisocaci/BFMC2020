...
class SteeringAngle(Thread):

    def __init__(self, inP, outQs):
        """
        outQs[0]: to control (sends the angle)
        outQs[1]: to speed (sends nb of lines)
        """
    ...
    def get_steering_angle(self, img_shape, local_lines):
        ...
        if local_lines[0] is not None and local_lines[1] is not None:
            ...
            distFromRight = abs(rslope * width / 2 - height - rx1 * rslope + ry1) / math.sqrt(rslope * rslope + 1)
            distFromLeft = abs(lslope * width / 2 - height - lx1 * lslope + ly1) / math.sqrt(lslope * lslope + 1)

            lines = 2
            self.outQs[1].put((lines, int(distFromRight), int(distFromLeft)))
            ...
            y2 = local_lines[1][3]
            slope = (y2-y1)/(x2-x1)
            distFromLeft = abs(slope*width/2 - height - x1*slope + y1) / math.sqrt(slope*slope + 1)
            lines = 11
            self.outQs[1].put((lines, None, int(distFromLeft)))
            ...
            y2 = local_lines[0][3]
            slope = (y2 - y1) / (x2 - x1)
            distFromRight = abs(slope * width / 2 - height - x1 * slope + y1) / math.sqrt(slope * slope + 1)
            lines = 10
            self.outQs[1].put((lines, int(distFromRight), None))

        elif local_lines[0] is None and local_lines[1] is None:
            self.outQs[1].put((0, None, None))
    ...
    def run(self):
        ...
        while True:
            ...
            try:
                ...
                self.outQs[0].put(self.steering_angle)
